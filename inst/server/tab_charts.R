...
emissions <- reactive({
  ...
  # Aggregate
  group_cols <- c(input$group_by, input$color_by, "year", "iso3", "region_name")
  e %>%
  ...
  if(chart_type=="barh"){
    ...
    plt <- e_plt %>%
      ggplot(aes(value, group)) +
       geom_bar(aes(fill=reorder(color, value),text=paste(paste0(color_display), sprintf("%.2f kt", value), sep="\n")), stat="identity") +
